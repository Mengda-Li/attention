...
1. **Empirical side.** We added a direct, head-resolved comparison between the real attention spectrum and the spectral-canonical attention spectrum. For every sample, layer, and head, we reconstruct
   ...
   Every one of the last five layers shows strong convergence with sequence length: by \(\ell=1024\), the mean distances lie between \(0.0829\) and \(0.1054\). In the aggregate figure, the all-layer median decreases from \(0.3002\) to \(0.0942\), and the last-layer median decreases from \(0.3290\) to \(0.0748\). This is the requested quantitative evidence that the spectral discrepancy decreases in the long-sequence regime.

   ![GPT-2 W1 distance between the empirical singular-value measures of real and spectral-canonical attention](https://anonymous.4open.science/r/attention-4ECD/rebuttal/softmax_LSD_distance/gpt2_w1_all_and_last_layer_vs_seq_len.png)

   PNG: https://anonymous.4open.science/r/attention-4ECD/rebuttal/softmax_LSD_distance/gpt2_w1_all_and_last_layer_vs_seq_len.png
   ...
   All layer medians are below \(0.104\), and 10 of 12 are below \(0.075\). The IQRs also display the layer/head heterogeneity rather than hiding it through a single average.

   ![ViT layer-wise W1 distance between the empirical singular-value measures of real and spectral-canonical attention](https://anonymous.4open.science/r/attention-4ECD/rebuttal/softmax_LSD_distance/vit_w1_by_layer.png)

   PNG: https://anonymous.4open.science/r/attention-4ECD/rebuttal/softmax_LSD_distance/vit_w1_by_layer.png
   ...
   The very narrow ranges of the ViT layer means show that these low discrepancies hold throughout the network, rather than arising from a favorable subset of layers or heads.

   ![GPT-2 Haar-invariance diagnostics for the left and right singular-vector matrices of the actual score](https://anonymous.4open.science/r/attention-4ECD/rebuttal/invariance_check_S/gpt2_svd_distance_metrics_three.png)

   PNG: https://anonymous.4open.science/r/attention-4ECD/rebuttal/invariance_check_S/gpt2_svd_distance_metrics_three.png

   ![ViT Haar-invariance diagnostics across layers for the left and right singular-vector matrices of the actual score](https://anonymous.4open.science/r/attention-4ECD/rebuttal/invariance_check_S/vit_svd_layer_distance_boxplot.png)

   PNG: https://anonymous.4open.science/r/attention-4ECD/rebuttal/invariance_check_S/vit_svd_layer_distance_boxplot.png
...
2. **Mathematical side.** Assumption 1 is the diagonalization-in-law hypothesis
   ...
   We will state the independence hypothesis explicitly. Since row-wise softmax is a deterministic smooth map, Assumption 1 implies through the continuous mapping theorem that
   ...
   The new \(W_1\) experiment above directly verifies this predicted post-softmax weak-LSD consequence on pretrained models.
...
![GPT-2 score singular values by layer with the empirical high-value group highlighted](https://anonymous.4open.science/r/attention-4ECD/rebuttal/score_bulk_spike/gpt2_score_svd_spike_bulk_by_layer_log_empirical_gap.png)

GPT-2 PNG: https://anonymous.4open.science/r/attention-4ECD/rebuttal/score_bulk_spike/gpt2_score_svd_spike_bulk_by_layer_log_empirical_gap.png

GPT-2 SVG: https://anonymous.4open.science/r/attention-4ECD/rebuttal/score_bulk_spike/gpt2_score_svd_spike_bulk_by_layer_log_empirical_gap.svg

![ViT score singular values by layer with the empirical high-value group highlighted](https://anonymous.4open.science/r/attention-4ECD/rebuttal/score_bulk_spike/vit_score_svd_spike_bulk_by_layer_log_empirical_gap.png)

ViT PNG: https://anonymous.4open.science/r/attention-4ECD/rebuttal/score_bulk_spike/vit_score_svd_spike_bulk_by_layer_log_empirical_gap.png

ViT SVG: https://anonymous.4open.science/r/attention-4ECD/rebuttal/score_bulk_spike/vit_score_svd_spike_bulk_by_layer_log_empirical_gap.svg

Related spiked-model reference:
https://proceedings.mlr.press/v119/seddik20a/seddik20a.pdf

**Conclusion:** Actual GPT-2 and ViT score matrices exhibit a practical bulk-spike morphology across the tested layers and heads, while the exact \(\log\ell\) separation remains the asymptotic assumption rather than an imposed finite-sample cutoff.
...
1. **Empirical side.** Yes. We added exactly this experiment on pretrained GPT-2 and ViT-B/16. For every sample, layer, and head, the SVDs of \(A_{{\rm real},h}=\operatorname{softmax}_{\rm row}(S_h+M)\) and \(A_{\Sigma,h}=\operatorname{softmax}_{\rm row}(\Sigma_h+M)\) are computed independently. We then calculate their bin-free \(W_1\) distance before any aggregation.
   ...
   GPT-2 uses 12 WikiText-103 documents, seven lengths, 12 layers, and 12 heads, giving 12,096 matched comparisons. The figure reports the all-layer and last-layer medians with IQRs; the table makes the convergence in the last five layers explicit. Each entry is mean \(W_1\pm\) standard deviation over 12 documents \(\times\) 12 heads.
   ...
   All five layers show strong convergence; their mean \(W_1\) values are only \(0.0829\)--\(0.1054\) at \(\ell=1024\). In the aggregate figure, the all-layer median decreases from \(0.3002\) to \(0.0942\), and the last-layer median decreases from \(0.3290\) to \(0.0748\).
   ...
   All ViT layer medians are below \(0.104\), with 10 of 12 below \(0.075\). The tables and figures therefore directly show both the agreement and its layer/head heterogeneity.
...
2. **Mathematical connection.** The experiment compares the two empirical singular-value measures
   ...
   Hence, whenever either sequence has an LSD, the other converges weakly to the same LSD. This experiment therefore directly tests the asymptotic weak-LSD prediction of the canonical model rather than using hidden-state isotropy or rank proxies as an indirect substitute.
...
1. **Empirical side.** We added log-scale histograms of the actual score singular values for both model types. Each of the 12 subfigures corresponds to one layer; within each layer, the SVD is computed separately for all 12 heads before pooling the nonzero singular values. The GPT-2 figure uses a 1024-token WikiText input, and the ViT-B/16 figure uses a 577-token Imagenette input. The empirical high-value group is colored in each panel, showing the lower bulk and larger upper group while retaining the full dynamic range.

   This is supported at dataset scale by Experiment 4.2 and Figure 3: over WikiText-103 validation sequences with \(17\le\ell\le471\), stable rank stays below approximately \(2\) and effective rank below approximately \(30\), without substantial growth with \(\ell\). The direct histograms show the separation, while the dataset-level rank proxies show that the number of dominant directions remains finite/moderate as length increases.
...
**Conclusion:** The separation is observed across all tested layers, individual heads, and both model types, with the bounded rank proxies further showing that the dominant spectral dimension remains finite and moderate as sequence length grows.

## Q3: Why should singular values alone control the spectrum after row-wise softmax?

1. **Empirical side.** We now test this question directly rather than relying only on motivation. As shown in the \(W_1\) figures placed directly under Q1 above, the per-head spectral distance decreases from \(0.3002\) to \(0.0942\) after pooling all GPT-2 layers and from \(0.3290\) to \(0.0748\) in the last layer as \(\ell\) grows from \(64\) to \(1024\). For ViT, all layer medians are below \(0.104\).
   ...
   Thus the left and right score singular-vector matrices behave almost identically, GPT-2 becomes increasingly Haar-like with sequence length, and the ViT discrepancies are uniformly low across layers.

   ![GPT-2 Haar-invariance diagnostics for actual-score left and right singular-vector matrices](https://anonymous.4open.science/r/attention-4ECD/rebuttal/invariance_check_S/gpt2_svd_distance_metrics_three.png)

   PNG: https://anonymous.4open.science/r/attention-4ECD/rebuttal/invariance_check_S/gpt2_svd_distance_metrics_three.png

   ![ViT Haar-invariance diagnostics across layers for actual-score left and right singular-vector matrices](https://anonymous.4open.science/r/attention-4ECD/rebuttal/invariance_check_S/vit_svd_layer_distance_boxplot.png)
...
2. **Mathematical side.** The diagonalization-in-law hypothesis is
   ...
   Under this stated hypothesis, applying the deterministic row-softmax map gives
...
1. **Actual score matrices.** For every layer and head, we reconstruct \(S_h=Q_hK_h^\top/\sqrt{d_h}\), compute its SVD independently, and test both \(U_{S_h}\) and \(V_{S_h}\) against the Haar reference. The numerical tables and aggregate figures are reported immediately in Q3 above; the links below additionally provide every GPT-2 layer separately for both sides and all three metrics.

2. **Actual attention matrices.** For the same head, we independently construct and decompose the real attention \(\operatorname{softmax}_{\rm row}(S_h+M)\) and its spectral-canonical counterpart \(\operatorname{softmax}_{\rm row}(\Sigma_h+M)\). The W1 figures above summarize 12,096 GPT-2 and 2,880 ViT matched per-head comparisons.

3. **Actual score spectra.** The highlighted log-scale score histograms likewise compute the SVD independently within each head and pool singular values only afterward within a layer.
...
Direct real-versus-canonical attention spectra:
...
Direct score spectra, highlighted log scale:
...
https://anonymous.4open.science/r/attention-4ECD/rebuttal/score_bulk_spike/gpt2_score_svd_spike_bulk_by_layer_log_empirical_gap.svg

https://anonymous.4open.science/r/attention-4ECD/rebuttal/score_bulk_spike/vit_score_svd_spike_bulk_by_layer_log_empirical_gap.svg

Actual-score singular-vector summaries:
...
GPT-2 per-layer actual-score diagnostics:

https://anonymous.4open.science/r/attention-4ECD/rebuttal/invariance_check_S/per_layer/gpt2_left_kl_divergence_convergence_log.png

https://anonymous.4open.science/r/attention-4ECD/rebuttal/invariance_check_S/per_layer/gpt2_right_kl_divergence_convergence_log.png

https://anonymous.4open.science/r/attention-4ECD/rebuttal/invariance_check_S/per_layer/gpt2_left_kolmogorov_distance_convergence_log.png

https://anonymous.4open.science/r/attention-4ECD/rebuttal/invariance_check_S/per_layer/gpt2_right_kolmogorov_distance_convergence_log.png

https://anonymous.4open.science/r/attention-4ECD/rebuttal/invariance_check_S/per_layer/gpt2_left_wasserstein_distance_convergence_log.png

https://anonymous.4open.science/r/attention-4ECD/rebuttal/invariance_check_S/per_layer/gpt2_right_wasserstein_distance_convergence_log.png

**Conclusion:** Yes—we now report actual per-head score matrices, real and spectral-canonical attention matrices, score spectra, and singular-vector diagnostics, aggregating only scalar results after each head has been decomposed independently.

# Response to Reviewer XnLy

## Q1: When does isotropy of \(X\) imply the required isotropy of \(S\)?

1. **Empirical side.** We agree that testing \(X\) alone leaves a gap, so the new experiments bypass the inheritance argument and directly decompose every actual per-head score matrix \(S_h=U_{S_h}\Sigma_hV_{S_h}^\top\). We compare the eigen-angle laws of both \(U_{S_h}\) and \(V_{S_h}\) with the circular-uniform Haar reference using KL divergence, Kolmogorov distance, and angular \(W_1\), all of which are minimized at zero.
   ...
   The left and right results are nearly identical. GPT-2 becomes increasingly Haar-like as \(\ell\) grows, while ViT remains uniformly close to the Haar reference across all layers. Thus the score-level premise required by the theory is tested on the actual \(S_h\), not inferred only from \(X\).
...
2. **Mathematical side.** If \(W_Q\) and \(W_K\) are fixed or independent of the random input \(X\), a token-space rotation \(X\mapsto OX\) gives
   ...
   We will explicitly state this condition on \(W_Q,W_K\), together with the independence hypothesis between the score singular values and singular vectors. The direct score experiments then verify the required behavior on the actual \(S_h\), rather than relying only on inheritance from \(X\).

**Conclusion:** Isotropy of \(X\) passes to the token-space rotation law of \(S\) when \(W_Q,W_K\) are fixed or independent of \(X\), while the new score-level experiments verify the required behavior directly rather than relying on this inheritance argument alone.

## Q2: Do encoder and decoder spectra degenerate at different rates?

1. **Empirical side.** ViT sequence length cannot be swept asymptotically without changing the model setup, so we cannot make the same length-convergence plot as for GPT-2. Nevertheless, Figures 4 and 5 compare encoder and decoder attention at similar lengths: ViT-B/16 has \(\ell=577\), and the longest GPT-2 WikiText example has \(\ell\approx470\). The encoder bulk is already very thin and concentrated at a substantially smaller scale from Layer 1, whereas the decoder retains a visibly broader bulk across layers. These existing figures provide empirical evidence consistent with faster encoder degeneration.

2. **Mathematical side.** We derive explicit rank-proxy results. Under the theorem's aggregate-tail condition and finite or sublinear \(k\), bidirectional attention satisfies

   \[
   \operatorname{srank}(A),\ \operatorname{erank}_1(A),\
   \operatorname{erank}_2(A)
   =(k+1)(1+o(1)).
   \]

   For causal attention with bounded bulk scores, the stable-rank proxies satisfy

   \[
   \operatorname{srank}(A),\ \operatorname{srank}_{\lambda}(A)
   =\Theta_B\!\left(k+\log\frac{\ell+1}{k+1}\right).
   \]

   The theorem also quantifies the entropy effective-rank proxies: the squared-singular-value and eigenvalue entropy proxies have the same logarithmic scale under the stated additional regularity condition, while \(\operatorname{erank}_1\) requires stronger tail control for a sharp equivalent. Thus both empirical spectral distributions degenerate, but for finite \(k\) the decoder retains a logarithmically diffuse effective bulk dimension and degenerates more slowly. We will add these rank-proxy theorems and proofs in the revision.

**Conclusion:** Both the matched-length empirical spectra and the rank-proxy theory support faster degeneration for bidirectional encoder attention, whereas causal decoder attention retains a logarithmically diffuse bulk and therefore degenerates more slowly.

## Q3: Are the assumptions applied per head or after aggregating heads?

1. **Empirical side.** The original Figures 4–5 aggregate attention heads before computing the attention SVD. In contrast, all new score-spectrum, actual-score Haar, and real-versus-canonical attention experiments compute every SVD independently per head and aggregate only the resulting distances or histogram values. The new results therefore show that the motivating empirical properties also occur at the individual-head level.

2. **Mathematical and scope side.** The present paper does not explicitly analyze the full multi-head attention block; the original spectral plots are aggregated-head diagnostics. At the same time, each individual head satisfies
   ...
   so every per-head score matrix is itself structurally low rank. We will explicitly distinguish the original aggregated-head figures from the new per-head diagnostics. A theory incorporating head concatenation, \(W_O\), normalization, and residual composition is a direction we are pursuing in follow-up work.

Per-head score spectra:
...
2. **Mathematical side.** Every head also obeys the structural bound
   ...
   with \(d_h=64\) for both tested checkpoints. Thus a fixed head dimension provides an \(\ell\)-independent upper bound, while Figure 3 shows that the realized effective dimension is substantially smaller. Estimating the exact number of spikes is a longstanding model-selection problem in spiked random-matrix statistics. Our degeneration results require \(k\) to remain finite or sublinear but do not require an exact estimator of \(k\).

**Conclusion:** The head dimension gives the structural bound \(k\le d_h\), and the observed stable/effective ranks show that the realized \(k\) remains much smaller and does not grow substantially with sequence length over the tested range.
